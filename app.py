...
def cargar_datos_gsheets():
    """
    Carga los datos desde Google Sheets. Implementa un caché de respaldo en session_state
    para garantizar resiliencia total de la interfaz ante eventuales pestañeos de conexión.
    """
    try:
        ...
        if df_proceso_raw is not None and not df_proceso_raw.empty:
            st.session_state["cache_df_proceso"] = df_proceso_raw
        if df_notas_raw is not None and not df_notas_raw.empty:
            st.session_state["cache_df_notas"] = df_notas_raw

    except Exception:
        # Recuperación fluida mediante caché ante fallos momentáneos de red
        df_proceso_raw = st.session_state.get("cache_df_proceso", pd.DataFrame())
        df_notas_raw = st.session_state.get("cache_df_notas", pd.DataFrame())
    ...
    try:
        ...
        return None, None, f"Error al procesar estructura de datos: {str(e)}"
...
def render_tablero_fluido():
    ...
    # 2. TABLA PRINCIPAL CON CONTROLES Y BUSCADOR MULTICRITERIO
    if df_vista is not None and not df_vista.empty:
        ...
        with col_search:
            search_val = st.text_input(
                "Buscar Orden o Responsable",
                value=st.session_state.get("search_term", ""),
                placeholder="🔍 Buscar Orden o Responsables...",
                key="search_input_widget",
                label_visibility="collapsed",
            )
            st.session_state.search_term = search_val

        # Búsqueda ampliada por número de orden y responsables (ej. buscar "NA")
        if st.session_state.search_term.strip():
            term = st.session_state.search_term.strip().lower()
            col_target_ord = "# Orden" if "# Orden" in df_vista.columns else df_vista.columns[0]
            col_target_resp = "Responsables" if "Responsables" in df_vista.columns else None

            mask = df_vista[col_target_ord].astype(str).str.lower().str.contains(term, na=False)
            if col_target_resp:
                mask |= df_vista[col_target_resp].astype(str).str.lower().str.contains(term, na=False)

            df_vista = df_vista[mask]

        # CÁLCULOS DE PAGINACIÓN Y TIEMPOS INTELIGENTES
        filas_por_pagina = 10
...
render_tablero_fluido()from datetime import datetime
import time
import urllib.parse
import numpy as np
import pandas as pd
