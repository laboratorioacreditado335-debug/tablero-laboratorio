    ...
    try:
        ...
        if df_proceso_raw is not None and not df_proceso_raw.empty:
            ...
            for idx, row in df_proceso_raw.iterrows():
                ...
                if "FECHA" in row_str or "ORDEN" in row_str or "CER" in row_str:
                    header_idx = idx
                    break
            ...
            # Identificar columna de Fecha y realizar ffill para celdas combinadas
            col_fecha = next(
            ...
            if col_fecha:
                ...
                df_proceso["Fecha"] = df_proceso["Fecha"].apply(
                    lambda x: np.nan
                    if pd.isna(x)
                    or str(x).strip().lower() in ["", "nan", "none", "null", "nat"]
                    else x
                )
...
def render_tablero_fluido():
    ...
    if df_main is not None and not df_main.empty:
        # Mapeo dinámico y flexible de columnas
        mapa_cols = {}
        ...
        # Si Fecha aún no está mapeada, asignar la primera columna
        if "Fecha" not in df_main_renamed.columns and len(df_main_renamed.columns) > 0:
        ...
        if "Fecha" in df_vista.columns:
            ...
            # Filtrar las órdenes programadas para el día actual
            ...
            # Formatear la fecha para mostrarla limpia en el Tablero Principal (DD/MM/YYYY)
    ...
    with c_right:
        ...
        if df_bitacora is None or df_bitacora.empty:
            ...
            col_e = next(
                (c for c in df_bitacora.columns if "ESTADO" in str(c).upper()),
                None,
            )
