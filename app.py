...
def render_bitacora_card(nota):
    # Mapeo flexible de claves de columna
    def get_valor(posibles_claves, defecto=""):
        for clave in posibles_claves:
            if clave in nota and pd.notna(nota[clave]) and str(nota[clave]).strip() != "":
                return str(nota[clave]).strip()
        return defecto

    prioridad = get_valor(["Prioridad", "prioridad", "PRIORIDAD"], "Media").lower()
    titulo = get_valor(["Titulo", "Título", "titulo", "título", "Encabezado"], "Aviso")
    mensaje = get_valor(["Mensaje", "Descripcion", "Descripción", "descripcion", "descripción", "Nota", "nota"], "")
    hora = get_valor(["Hora", "hora", "HORA", "Fecha", "fecha"], "")

    if not mensaje and titulo == "Aviso":
        return False
...
def render_tablero_fluido():
    ...
    firm_count = len(df_ordenes[df_ordenes["Estado"] == "Firmada"]) if not df_ordenes.empty and "Estado" in df_ordenes.columns else 0
    env_count = len(df_ordenes[df_ordenes["Estado"] == "Enviada"]) if not df_ordenes.empty and "Estado" in df_ordenes.columns else 0
    pend_count = len(df_ordenes[df_ordenes["Estado"] == "Pendiente"]) if not df_ordenes.empty and "Estado" in df_ordenes.columns else 0
    ...
    with col_bitacora:
        ...
        if not df_notas.empty:
            for _, nota in df_notas.iterrows():
                if render_bitacora_card(nota):
                    notas_desplegadas += 1
        
        if notas_desplegadas == 0:
